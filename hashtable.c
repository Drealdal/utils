...
static inline int init_hash_node(hash_node_t **node); 
static inline int distroy_hash_node(hash_node_t **node);

int pomme_hash_init(int size,
	
	int(*h_func)(void *,u_int32),
	int(*c_func)(void *,void *),
	hashtable_t **hash)
{
	u_int32 flags = 0;
	int i = 0;
	if( *hash == NULL )
	{
		*hash = malloc(sizeof(hashtable_t));	
		...
		flags |= HASH_NEED_FREE;
	}else{
		// distory the hash table
	}
	hashtable_t *p_hash = *hash;
	p_hash->size = size;
...
int pomme_hash_int_longlong(int size, hashtable_t **hash)
{
