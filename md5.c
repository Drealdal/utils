...
/*  add by zhumeiqi */
void md5_hash(uint8_t digest[16],const uint8_t *buff, uint32_t len)
{
    md5ctx ctx;
    md5_init(&ctx);
...
void md5_print(FILE *fd, uint8_t digest[16])
{
    int i = 0;
    for( i = 0 ; i < 16 ; ++i )
    {
	fprintf(fd,"%02x",digest[i]); 
    }
}
int md5_cmp(uint8_t dig1[16], uint8_t dig2[16])
{
    int i = 0 ; 
    for( i = 0 ; i < 16 ; ++i )
    {
